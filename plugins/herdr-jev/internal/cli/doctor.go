package cli

import (
	"encoding/json"
	"fmt"
	"os"
	"sort"
	"text/tabwriter"

	"github.com/muthuishere/herdr-jev/internal/agentkind"
	"github.com/muthuishere/herdr-jev/internal/config"
	"github.com/muthuishere/herdr-jev/internal/herdrapi"
	"github.com/muthuishere/herdr-jev/internal/jev"
)

// check is one prerequisite and, when it fails, what to do about it.
//
// A remediation is not decoration. This plugin's failures are QUIET by construction —
// it is designed never to block, so a completely dead backend looks exactly like a
// plugin that is simply not doing much. `doctor` is the only thing standing between
// that and a user who thinks it works. Every check that can fail names the exact
// command that fixes it.
type check struct {
	Name   string `json:"name"`
	OK     bool   `json:"ok"`
	Detail string `json:"detail"`
	Fix    string `json:"fix,omitempty"`
}

func doctor(flags map[string]string) error {
	var checks []check
	add := func(c check) { checks = append(checks, c) }

	// 1. Config parses. Everything downstream is read from it.
	cfg, cerr := config.Load()
	if cerr != nil {
		add(check{"config", false, cerr.Error(), "fix or delete " + config.Path()})
		return report(checks, flags)
	}
	add(check{"config", true, config.Path(), ""})

	// 2. The bars. Getting these inverted is silent and expensive: the plugin would
	// still run, still log, and spend money in the wrong direction all day.
	add(check{"confidence bars", true, fmt.Sprintf(
		"upgrade %.2f · downgrade %.2f · answer %.2f — spending more is the cheap mistake, so its bar is the lowest",
		cfg.Policy.MinUpgradeConfidence, cfg.Policy.MinDowngradeConfidence, cfg.Answer.MinConfidence), ""})

	// 3. The short circuit, and which mode it is in. "It never answers anything" is
	// almost always this, and it is the first thing anyone will ask about.
	switch {
	case cfg.Answer.Shadow:
		add(check{"short circuit", true, "SHADOW: logs what it would have answered, dispatches to an agent anyway", ""})
	case cfg.Answer.Enabled:
		add(check{"short circuit", true, fmt.Sprintf("ON, answering above %.2f confidence", cfg.Answer.MinConfidence), ""})
	default:
		add(check{"short circuit", false, "off (the default: answering instead of invoking your agent is opt-in)",
			"set answer.shadow = true in " + config.Path() + " to watch it for a week, then answer.enabled = true"})
	}

	// 4. Are we inside Herdr? Dispatch cannot start an agent otherwise.
	if os.Getenv("HERDR_ENV") == "" {
		add(check{"herdr env", false, "HERDR_ENV is not set; this is not a Herdr pane",
			"run inside a Herdr pane, or start one with `herdr`"})
	} else {
		add(check{"herdr env", true, "HERDR_ENV=1, pane " + firstNonEmpty(herdrapi.SelfPane(), "unknown"), ""})
	}

	// 5. Can we reach the Herdr server? `pane list` is the call dispatch makes.
	if panes, perr := herdrapi.New().Panes(ctx()); perr != nil {
		add(check{"herdr server", false, perr.Error(),
			"check HERDR_SOCKET_PATH; a named session has its own socket"})
	} else {
		add(check{"herdr server", true, fmt.Sprintf("%d pane(s)", len(panes)), ""})
	}

	// 6. openjev — the one dependency, and the one that degrades silently.
	client, berr := backend(cfg)
	if berr != nil {
		fix := "start it: openjev serve"
		if cfg.OpenJEV.Mode == "spawn" {
			fix = "check openjev.bin in " + config.Path() + "; `openjev --version` must work"
		}
		add(check{"openjev", false, berr.Error(), fix})
	} else {
		defer client.Close()
		add(check{"openjev", true, "reachable at " + client.URL, ""})

		// Ready and READY are different questions, and so are their fixes: a
		// download needs patience, a failure needs a log.
		if r, rerr := client.Ready(ctx()); rerr != nil {
			fix := "wait; this is a one-time cost"
			if r.Phase == "failed" {
				fix = "read the openjev server log; it is not retrying its way out of this"
			}
			add(check{"openjev ready", false, r.Human(), fix})
		} else {
			add(check{"openjev ready", true, r.Human(), ""})
		}

		if info, ierr := client.Info(ctx()); ierr != nil {
			add(check{"openjev api", false, ierr.Error(),
				"upgrade whichever of openjev or herdr-jev is older; this client speaks v" +
					fmt.Sprint(jev.APIVersion)})
		} else {
			add(check{"openjev api", true, fmt.Sprintf("v%d · %s on %s · max_options %d",
				info.APIVersion, info.Model.Ref, info.Model.Device, info.Limits.MaxOptions), ""})
		}
	}

	// 7. The agent-kind mappings, printed in full.
	//
	// A vendor renaming a flag produces an agent that is silently never routed — the
	// plugin logs a decision, builds arguments nothing accepts, and the model never
	// changes. Making the mapping visible is the only defence available to us.
	kinds := make([]string, 0, len(cfg.Agents))
	for k := range cfg.Agents {
		kinds = append(kinds, k)
	}
	sort.Strings(kinds)
	if len(kinds) == 0 {
		add(check{"agent kinds", false, "no [agents.*] sections configured",
			"add one, e.g. [agents.claude] with tiers.fast/balanced/deep"})
	}
	for _, name := range kinds {
		k := cfg.Agents[name]
		detail := fmt.Sprintf("%s → %s/%s/%s", firstNonEmpty(k.ModelFlag, k.ModelEnv, "NO MODEL CONTROL"),
			k.Tiers.Fast, k.Tiers.Balanced, k.Tiers.Deep)
		if !agentkind.Known(name) {
			add(check{"agent " + name, false, "herdr cannot start an agent of this kind",
				"rename the section to one of: " + fmt.Sprint(agentkind.HerdrKinds())})
			continue
		}
		if k.ModelFlag == "" && k.ModelEnv == "" {
			add(check{"agent " + name, false, "no model_flag and no model_env: nothing to route",
				"set model_flag (e.g. \"--model\") under [agents." + name + "]"})
			continue
		}
		if k.Tiers.Fast == "" || k.Tiers.Balanced == "" || k.Tiers.Deep == "" {
			add(check{"agent " + name, false, "a tier has no model name: " + detail,
				"fill in all three of tiers.fast / tiers.balanced / tiers.deep"})
			continue
		}
		add(check{"agent " + name, true, detail, ""})
	}

	return report(checks, flags)
}

func report(checks []check, flags map[string]string) error {
	failed := 0
	for _, c := range checks {
		if !c.OK {
			failed++
		}
	}
	if flags["json"] != "" {
		b, _ := json.MarshalIndent(map[string]any{"ok": failed == 0, "checks": checks}, "", "  ")
		fmt.Println(string(b))
	} else {
		w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
		for _, c := range checks {
			mark := "ok  "
			if !c.OK {
				mark = "FAIL"
			}
			fmt.Fprintf(w, "%s\t%s\t%s\n", mark, c.Name, c.Detail)
			if !c.OK && c.Fix != "" {
				fmt.Fprintf(w, "\t\t  -> %s\n", c.Fix)
			}
		}
		w.Flush()
	}
	if failed > 0 {
		return fmt.Errorf("%d check(s) failed", failed)
	}
	return nil
}
